...
export function ComicReader() {
  const { currentBook, currentChapter, progress, updateProgress } = useAppStore();
  ...
  }, [nextPage, prevPage, goToPage, pages]);
  ...
  const readingMode = currentBook.reading_mode || "page";
      ...
      >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            style={{ color: currentPage > 0 ? "var(--text-secondary)" : "var(--text-tertiary)" }}
            onClick={prevPage}
          ...
          <button
            className="px-2 py-1.5 rounded-lg"
            style={{ color: currentPage < pages.length - 1 ? "var(--text-secondary)" : "var(--text-tertiary)" }}
            onClick={nextPage}
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-sm" style={{ color: "var(--text-tertiary)" }}>没有找到图片</div>
            </div>
          </div>
        ) : isWebtoon ? (
          <div className="h-full overflow-y-auto">
            <div className="max-w-3xl mx-auto">
              {pages.map((page) => (
                <img
                  key={page.index}
                  src={`asset://localhost/${page.image_path}`}
                  alt={page.file_name}
                  className="w-full"
                  style={{ display: "block" }}
                />
          ...
          <div className="flex items-center justify-center h-full">
            <img
              src={`asset://localhost/${pages[currentPage]?.image_path}`}
              alt={pages[currentPage]?.file_name}
