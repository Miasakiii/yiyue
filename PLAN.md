...
| 3.4.4 | ~~Calibre 桥接~~ | 🗺 5.3.3 | ✅ 本轮：ebook-convert 格式转换（azw3/mobi/azw/rtf → txt/epub，检测三通道 + UI 状态），转换后自动导入；元数据同步暂缓（需 calibredb 库路径）；IPC +2（共 82） | — |
| 3.4.5 | ~~WebDAV 同步范围扩展~~ | 🗺 5.3.4 | ✅ 本轮：payload 增 bookmarks/reading_profiles/settings；阅读目标迁 settings 表（localStorage 首启迁移）随同步；upsert last-write-wins；IPC +2（共 84） | — |
...
1. **顺序**：P0 ✅ → P1 ✅（除 2.1 人工回归）→ P2 ✅ 全部 8 项 → 体验修复轮（EPUB 乱码/沉浸/分栏/周报/预加载）→ 依赖安全 ✅（双审计 0 漏洞）
2. **意外收获**（非 PLAN 立项）：应用图标全套、护眼暖纸主题、沉浸阅读（F11 + 隐藏窗口栏）、分栏阅读模式、漫画预加载、阅读周报、EPUB GBK 编码修复
3. **剩余**：2.1 人工回归（~95 项 UI）；P3 架构项触发条件未到（store 556 行、书库 500+ 本时再做）；3.3 KOReader 暂缓
4. **每项完成标准**：代码 + 测试 + STATUS.md/FUTURE.md 勾选更新 + 提交信息标注对应条目号
